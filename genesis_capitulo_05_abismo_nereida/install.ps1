...
function Test-GenesisRepo([string]$Path) {
  ...
  return (
    ...
    (Test-Path -LiteralPath (Join-Path $Path "src\game\battleModel.js") -PathType Leaf)
  )
}
...
Execute informando o caminho manualmente, por exemplo:
.\install.ps1 -RepoRoot "C:\Projetos\Genesis" -Validate
"@
  }
}

$ResolvedRepo = Resolve-Path -LiteralPath $RepoRoot -ErrorAction Stop
$RepoRoot = $ResolvedRepo.Path

if (-not (Test-GenesisRepo $RepoRoot)) {
  throw "O caminho '$RepoRoot' não parece ser a raiz do Genesis. Verifique se package.json e src\game\content.js existem nessa pasta."
}
