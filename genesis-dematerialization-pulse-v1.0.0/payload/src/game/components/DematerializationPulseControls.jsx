...
function enemyOccupiesPulseRow(enemy, row) {
  ...
  if (enemy.type === "leviathanNereida") {
    return Boolean(enemy.leviathanTargetable && enemy.leviathanTargetableRows?.includes(row));
  }
  return enemy.row === row && !enemy.rasgamarSubmerged;
}

function routeTargets(session, row) {
  return (session?.enemies || []).filter((enemy) => enemyOccupiesPulseRow(enemy, row));
}

export function DematerializationPulseControls({ session, onActivate }) {
  if (!session || session.outcome || !(session.waveActive || session.sandbox)) return null;
  ...
  return (session.dematerializationPulses || []).map((pulse) => {
    ...
    const potentialDamage = targets.reduce(
      (total, enemy) => total + Math.min(Number(enemy.hp) || 0, DEMATERIALIZATION_PULSE.damage),
      0,
    );
    ...
    const cannonY = VIEWPORT.fieldOffsetY + pulse.row * CELL.height + CELL.height / 2;
    ...
    const status = charging
      ...
      : pulse.state === "spent" ? "USADO" : `${DEMATERIALIZATION_PULSE.damage}`;
    ...
    return (
      ...
      >
        <span className="pulse-control-icon" aria-hidden="true">⚡</span>
        <span className="pulse-control-value">{status}</span>
        <span className="pulse-control-label">{charging ? "CARREGANDO" : ready ? "DISPARAR" : "DESCARREGADO"}</span>
      </button>
    );
  });
}
