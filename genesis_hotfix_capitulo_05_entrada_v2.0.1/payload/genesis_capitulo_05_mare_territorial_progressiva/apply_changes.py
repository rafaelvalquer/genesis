...
def replace_tide_import(source: str) -> str:
    # Localiza apenas o bloco de importação imediatamente associado a tideCycle.js.
    # A expressão anterior começava no primeiro `import {` do arquivo e podia consumir
    # todos os imports intermediários, quebrando battleModel.js em tempo de execução.
    end_marker = '} from "./tideCycle.js";'
    end = source.find(end_marker)
    if end >= 0:
        start = source.rfind("import {", 0, end)
        if start < 0:
            fail("Encontrei tideCycle.js, mas não o início do respectivo import.")
        end += len(end_marker)
        return source[:start] + TIDE_IMPORT + source[end:]
