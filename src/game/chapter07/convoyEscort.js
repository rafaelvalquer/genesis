...
export function updateConvoyEscort(session, events = []) {
  ...
  if (wasEscorted !== session.convoy.escorted) events.push({ type: session.convoy.escorted ? "escortRestored" : "escortLost", convoyId: session.convoy.id });
  if (wasEscorted && !session.convoy.escorted && session.troops.some((troop) => session.elapsed < (troop.sporeConfusedUntil || 0))) {
