...
export function startConvoyEntry(session, events = []) { const convoy=session?.convoy; const flow=session?.convoyFlow; if(!convoy||!flow||flow.sectorIndex!==0||convoy.entryState!=="offscreen")return false; const stops=session.phase.convoy.sectorStops||[.06,.28,.51,.74,.96]; flow.state="convoyEntry"; convoy.entryState="entering"; convoy.x=convoy.entryX; convoy.previousX=convoy.entryX; convoy.entry={fromX:convoy.entryX,toX:getConvoyXForProgress(stops[0]),progress:0,startedAt:session.elapsed,durationMs:session.phase.convoy.entryDurationMs||convoy.entryDurationMs||2200}; convoy.animation={...(convoy.animation||{}),previousState:convoy.animation?.state||"idle",state:"run",startedAt:session.elapsed}; convoy.invulnerable=true; session.waveActive=false; session.queue=[]; session.preparing=false; events.push({type:"convoyEntryStarted",x:convoy.x,y:convoy.y}); return true; }
