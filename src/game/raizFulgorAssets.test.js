...
const ROOT = join(process.cwd(), "src", "game", "assets", "enemy", "raizFulgor");
const STATES = [
  "idle", "walking", "rooting", "rootedIdle", "attackCharge",
  "attackRelease", "unrooting", "stunned", "death",
];

describe("assets do Raiz-Fulgor", () => {
  it.each(STATES)("%s possui oito quadros RGBA 320x256 distintos e transparentes", async (state) => {
  ...
  it("nao possui estado hit e usa uma ancora terrestre comum nos 72 quadros", () => {
    expect(readdirSync(ROOT).sort()).toEqual([...STATES].sort());
    const anchors = ENEMY_FRAME_ANCHORS.raizFulgor;
    const sharedAnchors = new Set();
    STATES.forEach((state) => {
      expect(anchors[state]).toHaveLength(8);
      anchors[state].forEach((anchor) => sharedAnchors.add(`${anchor.x}:${anchor.y}`));
    });
