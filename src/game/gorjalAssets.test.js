...
const ROOT = join(process.cwd(), "src", "game", "assets", "enemy", "gorjal");
const STATES = [
  "idle", "walking", "attack", "chargePrep", "charge", "chargeImpact",
  "recover", "stunned", "death",
];

describe("assets do Gorjal", () => {
  ...
  it("não possui estados fora do contrato e compartilha uma âncora terrestre estável", () => {
    expect(readdirSync(ROOT).sort()).toEqual([...STATES].sort());
    const anchors = ENEMY_FRAME_ANCHORS.gorjal;
