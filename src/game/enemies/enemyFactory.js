...
export function createBaseEnemy(session, queued, config, createId) {
  ...
  const enemy = {
    ...
    sourceTreeId: queued.sourceTreeId || null,
    ...
    emergeUntil: null, animationState: queued.animationState || (queued.type === "larvaRaizFerro" ? "walking" : null),
    ...
    persistentBiteTargetId: null, persistentBiteHits: 0, persistentBiteMultiplier: 1, frenzyLevel: 0,
    saltadorState: queued.type === "saltadorAlado" ? "walking" : null,
    saltadorStateStartedAt: queued.type === "saltadorAlado" ? session.elapsed : -Infinity,
    saltadorStateEndsAt: Infinity, canopyJumpReadyAt: 0, jumpTargetId: null, jumpStartX: null,
    jumpLandingX: null, jumpAirStartedAt: null, escortInstinctActive: false, escortTargetId: null,
    escortTargetLockUntil: 0, rasanteReadyAt: 0, rasanteTargetId: null, rasanteImpactAt: Infinity,
    rasanteImpactApplied: false, airborne: false, visualOffsetY: 0, groundMeleeTargetable: true,
    groundRangedTargetable: true,
