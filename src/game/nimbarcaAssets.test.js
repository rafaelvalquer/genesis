...
const ROOT = join(process.cwd(), "src", "game", "assets", "enemy", "nimbarca");
const STATES = ["flying", "attack", "shieldPulse", "death"];

describe("assets do Nimbarca", () => {
  it.each(STATES)("%s possui oito quadros RGBA 384x384 distintos, transparentes e compactos", async (state) => {
    ...
    for (const file of files) {
      ...
      expect(metadata).toMatchObject({ width: 384, height: 384, channels: 4, hasAlpha: true });
  ...
  it("não possui estados visuais fora do contrato e compartilha uma âncora aérea estável", () => {
    expect(readdirSync(ROOT).sort()).toEqual([...STATES].sort());
    const anchors = ENEMY_FRAME_ANCHORS.nimbarca;
    ...
    STATES.forEach((state) => {
      ...
      expect(anchors[state][0].y).toBeGreaterThan(0.3);
