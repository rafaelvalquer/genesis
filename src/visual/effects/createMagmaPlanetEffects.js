...
export function createMagmaPlanetEffects({ THREE, profile }) {
  ...
  const lava = createRouteRibbon({ THREE, chapterId: "chapter_06", radius: 1.028, width: .085, material: new THREE.MeshBasicMaterial({ color: "#ff5b14", transparent: true, opacity: .54, depthWrite: false, side: THREE.DoubleSide, blending: THREE.AdditiveBlending }) });
  root.add(lava);
  ...
  root.userData.update = (_delta, elapsed, reduceMotion) => { lava.material.opacity = .46 + (reduceMotion ? .04 : (Math.sin(elapsed * .002) + 1) * .1); embers.rotation.y += reduceMotion ? 0 : .00035; };
  return root;
}
