...
import { getConvoyAnimationLabEntries, loadAnimationEntity, releaseAnimationEntityAssets } from "./animationLabAssetLoader.js";
...
const labelFor = (state) => state.replace(/_/g, " ").replace(/([a-z])([A-Z])/g, "$1 $2").replace(/^./, (char) => char.toUpperCase());
const troopEntries = Object.values(TROOPS);

function timingFor(asset, state, frames) {
  const convoy = asset?.type === "convoy" ? CONVOY_ANIMATION_CONFIG[state] : null;
  if (convoy) return { frameMs: convoy.frameDuration, durationMs: convoy.frames * convoy.frameDuration, impactFrame: null, impactMs: null };
...
export default function AnimationLabPage() {
  ...
  const entries = category === "troops" ? troopEntries : category === "convoy" ? getConvoyAnimationLabEntries() : getEnemyCatalogEntries();
  ...
  useEffect(() => {
    ...
    loadAnimationEntity({ type: category === "troops" ? "troop" : category === "convoy" ? "convoy" : "enemy", id: selected?.id, signal: controller.signal }).then((result) => setAsset(result)).catch((loadError) => { if (loadError.name !== "AbortError") setError(loadError.message); });
  ...
  const selectCategory = (next) => { setCategory(next); const nextEntries = next === "troops" ? troopEntries : next === "convoy" ? getConvoyAnimationLabEntries() : getEnemyCatalogEntries(); setSelectedId(nextEntries[0]?.id); };
  return <main className="page-content animation-lab-page">
    ...
    <div className="animation-lab-tabs" role="tablist" aria-label="Categorias do laboratório">{[["troops", "Tropas"], ["enemies", "Monstros"], ["convoy", "Transportes"]].map(([id, label]) => <button key={id} role="tab" aria-selected={category === id} className={category === id ? "active" : ""} onClick={() => selectCategory(id)}>{label}</button>)}</div>
