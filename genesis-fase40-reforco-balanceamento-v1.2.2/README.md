# Fase 40 — Reforço e balanceamento v1.2.2
...
## Correção da validação na v1.2.2

O script de auditoria dos sprites do Leviatã encerra com código `1` quando encontra
frames marcados para redesenho. No estado atual do repositório, ele informa 90 frames.
Essa pendência visual é anterior às alterações da Fase 40 e fazia o instalador v1.2.0
interpretar a auditoria como falha de compilação e restaurar o backup.

A v1.2.2 executa a auditoria de forma informativa e continua somente quando um relatório
válido confirma que o código `1` veio das marcações visuais conhecidas. Em seguida, ela
executa separadamente:

- toda a suíte de testes;
- o build de produção do Vite;
- a verificação geral de assets;
- a validação dos frames do Crisálio.

Falhas reais nessas etapas continuam cancelando a instalação e restaurando o backup.


## Correção da v1.2.2

Corrige o erro do PowerShell em que o operador `-f` era interpretado como um
parâmetro do comando `Write-Warning`. A mensagem agora é formatada primeiro e
enviada ao comando em seguida.
...
```powershell
cd "C:\Projetos\Genesis\genesis-fase40-reforco-balanceamento-v1.2.2"
