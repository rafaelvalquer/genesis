...
function getCompletedProgressIndex(phaseStats) {
  return PHASES.reduce((highest, phase, index) => {
    const victories = Number(
      phaseStats?.[phase.id]?.victories || 0,
    );

    if (victories <= 0) return highest;
...
export function migrateSave(value) {
  ...
  const phaseStats = isPlainObject(value.phaseStats)
    ? value.phaseStats
    : {};
  const completedLegacyCampaign = (
    Number(phaseStats.fase_08?.victories || 0) > 0
  );
  const completedProgressIndex = getCompletedProgressIndex(
    phaseStats,
  );
  const migratedIndex = Math.max(
    Number(value.unlockedPhaseIndex) || 0,
    completedLegacyCampaign ? 8 : 0,
...
export function loadCampaign(storage = window.localStorage) {
  try {
    return migrateSave(
      JSON.parse(storage.getItem(SAVE_KEY) || "null"),
    );
  } catch {
...
export function recordBattleResult(
  ...
  const next = {
    ...migrateSave(save),
    unlockedPhaseIndex: won
      ? Math.max(
        save.unlockedPhaseIndex,
        Math.min(PHASES.length - 1, phaseIndex + 1),
      )
      : save.unlockedPhaseIndex,
    currentPhaseId: won
      ? PHASES[
    ...
    phaseStats: {
      ...
      [result.phaseId]: {
        ...
        victories: (
          Number(previous.victories || 0)
          + (won ? 1 : 0)
        ),
        bestStars: Math.max(
          Number(previous.bestStars || 0),
        ...
        bestTimeMs: (
          ...
          && (
            !previous.bestTimeMs
            || result.durationMs < previous.bestTimeMs
          )
        )
          ? result.durationMs
          : previous.bestTimeMs || null,
        bestIntegrity: Math.max(
          Number(previous.bestIntegrity || 0),
          Number(result.integrity || 0),
        ),
