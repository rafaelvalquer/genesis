...
$Version = "dematerialization-pulse-v1.0.2"
...
Write-Host "== Genesis · Pulso de Desmaterialização Tático v1.0.2 ==" -ForegroundColor Cyan
...
if ($alreadyInstalled) {
  Write-Host "Implementação v1.0.0/v1.0.1/v1.0.2 já detectada. Patch de gameplay não será reaplicado." -ForegroundColor DarkGray
} else {
...
try {
  $validator = Join-Path $PackageRoot "validate.ps1"
  if ($FullValidation) {
