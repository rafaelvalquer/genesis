...
describe("fase 40 — defesa inicial reforçada", () => {
  it("declara três colunas completas de tropas bônus", () => {
    ...
    expect(phase.startingTroops.filter((entry) => entry.type === "bastiaoMare"))
      .toEqual(Array.from({ length: 5 }, (_, row) => ({ type: "bastiaoMare", row, col: 6 })));
    expect(phase.startingTroops.filter((entry) => entry.type === "fuzileiroVoltaico"))
      .toEqual(Array.from({ length: 5 }, (_, row) => ({ type: "fuzileiroVoltaico", row, col: 5 })));
    expect(phase.startingTroops.filter((entry) => entry.type === "medicaNanites"))
      .toEqual(Array.from({ length: 5 }, (_, row) => ({ type: "medicaNanites", row, col: 3 })));
  });
  ...
  it("não exige as tropas no loadout e preserva energia e Supply", () => {
    ...
    expect(session.providedTroops).toEqual({
      bastiaoMare: 5, fuzileiroVoltaico: 5, medicaNanites: 5,
    });
  ...
  it("aplica explicitamente o limite cinco às três tropas fornecidas", () => {
    const phase = phase40();
    ...
    for (const troopId of ["bastiaoMare", "fuzileiroVoltaico", "medicaNanites"]) {
      expect(getActiveTroopCount(session, troopId)).toBe(5);
      expect(getTroopDeploymentLimit(troopId, session)).toBe(5);
      expect(canPlaceTroop(session, troopId, 0, 2)).toContain("Limite de 5");
    }
  });
  ...
  it("cobra normalmente pela reposição de uma tropa escolhida no loadout", () => {
    const session = createBattleSession(phase40(), ["bastiaoMare"], 4040);
