# Genesis — Final de onda cinematográfico v2.0.1
...
## Reinstalação após falha do v2.0.0

Se você já executou o `v2.0.0` e recebeu `Patch incompatível: snapshot público do último abate`, **não restaure nada**. A tentativa anterior pode ter copiado os módulos novos, mas não concluiu os patches dos arquivos principais.

Extraia esta versão `v2.0.1` em uma pasta nova e execute diretamente em uma linha:

```powershell
.\install.ps1 -RepoRoot "C:\Projetos\Genesis" -FullValidation
```

O patcher do `v2.0.1` é idempotente: reconhece arquivos/módulos já copiados, aceita CRLF ou LF e localiza o bloco de `lastKill` dentro de `getSnapshot()` por estrutura em vez de exigir texto idêntico. Não é necessário apagar `.genesis-backups` nem os arquivos `src/game/waveOutro` já criados.
...
```powershell
cd "C:\Projetos\Genesis\genesis-wave-outro-cinematic-v2.0.1"
...
```text
.genesis-backups\wave-outro-cinematic-v2.0.1\<timestamp>
```
