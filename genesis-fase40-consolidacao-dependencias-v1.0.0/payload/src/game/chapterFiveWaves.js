...
function normalizeWaveOptions(input) {
  if (Number.isInteger(input)) {
    return {
      phaseIndex: input,
      phaseId: `fase_${String(33 + input).padStart(2, "0")}`,
      finalMission: input === PHASE_40_SCENARIO.finalPhaseIndex,
    };
  }

  const phaseIndex = Number(input?.phaseIndex);
  if (!Number.isInteger(phaseIndex) || phaseIndex < 0) {
    throw new TypeError("createChapterFiveWaves requer phaseIndex inteiro e não negativo.");
  }

  return {
    phaseIndex,
    phaseId: input?.phaseId,
    finalMission: isPhase40Scenario(input),
  };
}
