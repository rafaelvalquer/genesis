...
function patchChapterFiveContentTest(original) {
  const oldBlock = `      expect(phasePressure).toBeGreaterThan(previousPressure);
      previousPressure = phasePressure;`;
  const newBlock = `      if (phase.id !== "fase_40") {
        expect(phasePressure).toBeGreaterThan(previousPressure);
      }
      previousPressure = phasePressure;`;

  if (original.includes(newBlock)) return [original, []];
  const count = occurrenceCount(original, oldBlock);
  if (count !== 1) {
    throw new PatchError(
      `chapterFiveContent.test.js/pressão especial da fase 40: esperado 1 marcador, encontrado ${count}.`,
    );
  }
  return [
    original.replace(oldBlock, newBlock),
    ["teste global atualizado para a defesa inicial especial da fase 40"],
  ];
}
...
function main() {
  ...
  const patches = [
    ...
    ["src/game/chapterFiveContent.test.js", patchChapterFiveContentTest],
  ];
