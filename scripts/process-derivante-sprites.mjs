...
const tempRoot = path.join(root, ".codex-tmp", "derivante");
const rawRoot = path.join(tempRoot, "raw");
const keyedRoot = path.join(tempRoot, "keyed");
const outputRoot = path.join(root, "src", "game", "assets", "enemy", "derivante");
const sheetRoot = path.join(root, "artifacts", "derivante", "sheets");
const states = [
...
async function isolateLargestComponent(file) {
  ...
  // Some generated sheets contain a thin white cell divider. Removing the
  // outer pixels before component analysis prevents it from becoming an asset.
...
async function keyFrames() {
  const python = process.env.DERIVANTE_PYTHON || "python";
  const helper = process.env.DERIVANTE_CHROMA_HELPER;
  if (!helper) throw new Error("Set DERIVANTE_CHROMA_HELPER to remove_chroma_key.py");
  const work = [];
  for (const state of states) {
    for (let index = 0; index < 8; index += 1) {
...
else throw new Error("Use: node scripts/process-derivante-sprites.mjs extract|key|finalize");
