...
async function preview(frames) {
  ...
  await sharp(checkerboard(width, height), { raw: { width, height, channels: 4 } }).composite(composites).png().toFile(join(reports, "leviathan-sprite-audit-preview.png"));
}
...
await writeFile(join(reports, "leviathan-sprite-audit.json"), `${JSON.stringify(report, null, 2)}\n`); await writeFile(join(reports, "leviathan-sprite-audit.md"), markdown); await preview(frames.filter((frame) => frame.location === "source"));
