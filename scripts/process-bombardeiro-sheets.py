...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process Bombardeiro sprite sheets.")
    parser.add_argument("--state", choices=("idle", "attack", "both"), default="both")
    args = parser.parse_args()

    anchors = {}
    if args.state in ("idle", "both"):
        anchors["idle"] = split_sheet(SHEETS / "bombardeiro-idle.png", "idle")
    if args.state in ("attack", "both"):
        anchors["attack"] = split_sheet(SHEETS / "bombardeiro-attack.png", "attack")
    validate_frames()
    print(f"Bombardeiro sprites written to {TARGET}")
