...
const generatedAnchors = await buildAnchors();
// A targeted regeneration must not discard anchors previously generated for a
// different special enemy (for example Saltador Alado).
let anchors = generatedAnchors;
if (requested.size && process.argv.includes("--write")) {
  try {
    const existing = await import(`${pathToFileURL(OUTPUT).href}?anchors=${Date.now()}`);
    anchors = { ...(existing.ENEMY_FRAME_ANCHORS || {}), ...generatedAnchors };
  } catch { /* first targeted generation has no file to preserve */ }
}
const lines = [
