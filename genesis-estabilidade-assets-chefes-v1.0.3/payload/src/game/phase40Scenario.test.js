...
describe("cenário consolidado da Fase 40", () => {
  it("é a única fonte da defesa inicial e das regras da missão", () => {
    ...
    expect(phase.startingTroopRules).toBe(PHASE_40_SCENARIO.startingTroopRules);
    expect(phase.troopAssetDependencies).toBe(PHASE_40_SCENARIO.troopAssetDependencies);
    expect(phase.requiredTroopAssetIds)
      .toBe(phase.troopAssetDependencies.required);
    expect(PHASE_40_SCENARIO.requiredTroopAssetIds)
      .toBe(PHASE_40_SCENARIO.troopAssetDependencies.required);
  });

  it("declara cinco unidades de cada tipo nas colunas corretas", () => {
    const groups = PHASE_40_SCENARIO.startingDefense.reduce((result, entry) => {
      result[entry.type] ||= [];
      result[entry.type].push(entry);
      return result;
    }, {});
    expect(groups.bastiaoMare).toEqual(Array.from(
      { length: 5 }, (_, row) => ({ type: "bastiaoMare", row, col: 6 }),
    ));
    expect(groups.fuzileiroVoltaico).toEqual(Array.from(
      { length: 5 }, (_, row) => ({ type: "fuzileiroVoltaico", row, col: 5 }),
    ));
    expect(groups.medicaNanites).toEqual(Array.from(
      { length: 5 }, (_, row) => ({ type: "medicaNanites", row, col: 3 }),
    ));
  });

  it("remove as ondas legadas e congela os oito blueprints", () => {
    expect(CHAPTER_FIVE_PHASE_BLUEPRINTS).toHaveLength(8);
    expect(Object.isFrozen(CHAPTER_FIVE_PHASE_BLUEPRINTS)).toBe(true);
    expect(CHAPTER_FIVE_PHASE_BLUEPRINTS.every(Object.isFrozen)).toBe(true);
    expect(CHAPTER_FIVE_PHASE_BLUEPRINTS.every(
      (blueprint) => !Object.hasOwn(blueprint, "waves"),
    )).toBe(true);
  });

  it("mantém o contrato final e seus limites congelados", () => {
    ...
    expect(Object.isFrozen(
      PHASE_40_SCENARIO.startingTroopRules.deploymentLimits,
    )).toBe(true);
