...
describe("contrato de balanceamento da Fase 40", () => {
  ...
  it("rejeita índices fora das oito fases do capítulo", () => {
    expect(() => createChapterFiveWaves(-1)).toThrow(/entre 0 e 7/);
    expect(() => createChapterFiveWaves(8)).toThrow(/entre 0 e 7/);
  });

  it("rejeita configurações contraditórias da missão final", () => {
    expect(() => createChapterFiveWaves({
      phaseIndex: 7,
      phaseId: "fase_40",
      finalMission: false,
    })).toThrow(/Configuração inconsistente/);
    expect(() => createChapterFiveWaves({
      phaseIndex: 6,
      phaseId: "fase_40",
      finalMission: true,
