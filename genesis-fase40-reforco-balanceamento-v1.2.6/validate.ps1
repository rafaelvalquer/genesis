...
function Invoke-GenesisAssetValidation {
  ...
  foreach ($entry in $output) {
    $line = ([string]$entry).Trim().TrimStart([char]0xFEFF)
    if ([string]::IsNullOrWhiteSpace($line)) {
      continue
    }

    # O padrão usa somente caracteres ASCII para permanecer estável no
    # Windows PowerShell 5.1, mesmo quando o script for aberto sem BOM.
    if ($line -match '^Build excede.*82 MB: [0-9]+(?:[.,][0-9]+)? MB\.$') {
