# Fase 40 — Reforço e balanceamento v1.2.6
...
A versão 1.2.5 capturava corretamente o `stderr`, mas o arquivo PowerShell estava
codificado como UTF-8 sem BOM. O Windows PowerShell 5.1 interpreta esse formato
como ANSI. Dessa forma, o texto literal `orçamento` usado no padrão interno era
lido como `orÃ§amento`.

A saída do Node era lida corretamente como UTF-8:

```text
Build excede o orçamento total de 82 MB: 187.8 MB.
```

Porém, o padrão interno corrompido não correspondia à linha e ela era tratada
como problema desconhecido.

A v1.2.6 aplica duas proteções:

- `install.ps1` e `validate.ps1` são gravados como UTF-8 com BOM;
- o padrão do orçamento usa somente caracteres ASCII:

```powershell
'^Build excede.*82 MB: [0-9]+(?:[.,][0-9]+)? MB\.$'
```

Assim, a classificação funciona tanto no Windows PowerShell 5.1 quanto no
PowerShell 7.
...
```powershell
cd "C:\Projetos\Genesis\genesis-fase40-reforco-balanceamento-v1.2.6"
