...
if (-not (Test-Path (Join-Path $RepoRoot "package.json"))
  -or -not (Test-Path (Join-Path $RepoRoot "src\game\content.js"))
  -or -not (Test-Path (Join-Path $RepoRoot "src\game\battleModel.js"))) {
...
try {
  ...
  if ($LASTEXITCODE -ne 0) { throw "Falha ao aplicar as alterações." }
  ...
  if ($Validate) {
    ...
    if ($FullSuite) { $args.FullSuite = $true }
    & (Join-Path $PackageRoot "validate.ps1") @args
  }
