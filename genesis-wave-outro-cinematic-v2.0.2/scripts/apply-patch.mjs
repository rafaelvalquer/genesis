...
function patchGameCanvas() {
  ...
  // Imports: tolerate reordered imports and partially-applied previous versions.
  if (!content.includes('from "./waveOutro/waveOutroCamera.js"')) {
    const marker = 'import { installNonPassiveContextMenuGuard } from "./hooks/battleCanvasEvents.js";';
    if (content.includes(marker)) {
      ...
      note("imports waveOutro", "aplicado");
    } else {
      // Fallback: insert before the first local render import near the end of the import section.
      const fallback = 'import { drawSprite, drawSpriteInRect, getTroopVisualEntity } from "./render/battleSceneRenderer.js";';
      if (!content.includes(fallback)) throw new Error("Patch incompatível: seção de imports do GameCanvas");
      content = ensureImport(
        content,
        fallback,
        `import { getWaveOutroCameraTransform } from "./waveOutro/waveOutroCamera.js";\nimport {\n  getWaveOutroMusicVolume,\n  playWaveOutroImpactSound,\n  playWaveOutroVictoryStinger,\n} from "./waveOutro/waveOutroAudio.js";\nimport { getWaveOutroOverlayModel } from "./waveOutro/waveOutroRenderer.js";\n`,
      ...
      note("imports waveOutro (fallback)", "aplicado");
    }
  } else {
    // Older partial builds may contain only camera import. Ensure the other two modules too.
    if (!content.includes('from "./waveOutro/waveOutroAudio.js"')) {
      const marker = 'import { getWaveOutroCameraTransform } from "./waveOutro/waveOutroCamera.js";\n';
      content = ensureImport(
        content,
        marker,
        `import {\n  getWaveOutroMusicVolume,\n  playWaveOutroImpactSound,\n  playWaveOutroVictoryStinger,\n} from "./waveOutro/waveOutroAudio.js";\n`,
    ...
    if (!content.includes('from "./waveOutro/waveOutroRenderer.js"')) {
      const marker = 'import { getWaveOutroCameraTransform } from "./waveOutro/waveOutroCamera.js";\n';
      content = ensureImport(
        content,
        marker,
        `import { getWaveOutroOverlayModel } from "./waveOutro/waveOutroRenderer.js";\n`,
        'from "./waveOutro/waveOutroRenderer.js"',
      );
    }
    note("imports waveOutro", "já presente");
  }
  ...
  const desiredOverlayTag = '<WaveOutroOverlay outro={snapshot.waveOutro} phase={phase} reduceMotion={settings.reduceMotion} />';
  if (!content.includes(desiredOverlayTag)) {
    const broadTag = /<WaveOutroOverlay\b(?=[^>]*outro=\{snapshot\.waveOutro\})[^>]*\/>/m;
    if (broadTag.test(content)) {
      ...
      const pairedTag = /<WaveOutroOverlay\b(?=[^>]*outro=\{snapshot\.waveOutro\})[^>]*>\s*<\/WaveOutroOverlay>/m;
      if (pairedTag.test(content)) {
        content = content.replace(pairedTag, desiredOverlayTag);
        note("props do WaveOutroOverlay", "ajustado por fallback");
      } else {
        throw new Error("Patch incompatível: uso do WaveOutroOverlay com snapshot.waveOutro não localizado");
      }
    }
...
try {
  ...
  patchStyles();
  console.log("Patch cinematográfico de final de onda v2.0.2 aplicado. Nenhum rollback automático foi executado.");
} catch (error) {
